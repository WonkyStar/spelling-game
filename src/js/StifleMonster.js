export default class StifleMonster {
  constructor (stage, originalStageSize, x, y, life = 10 , delay, onDeath) {
    ...
    this.stifleSprite.scale.x = 0.15
    this.stifleSprite.scale.y = 0.15

    this.fromLeft = Math.random() > 0.5

    this.stifleSprite.anchor = {x: 0.5, y: 1}
    this.stifleSprite.position.x = window.innerWidth / 2 - 200
  ...
  onTap () {
    if (this.life > 1) {
      ...
      this.stifleSprite.scale.y -= 0.004
  ...
  animate () {
    if (this.stifleSprite.position.y < window.innerHeight || this.goDown) {
      this.goDown = true
